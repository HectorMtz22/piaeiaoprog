...
<body>
    ...
    <a href='consultar.php'>Actualizar tabla...</a>
		...
		<main class="grid-4">
			...
			<?php
                require "../classes/classVenta.php";
                if (!empty($_SESSION['isAdmin'])) {
                    $ventas = new ApiVentasAdmin;
                    $total = $ventas->getAll();
                } else {
                    $ventas = new ApiVentas;
                    $total = $ventas->getAll($_SESSION['idUsuario']);
                }
                foreach ($total["items"] as $clave => $valor) {
                    print "<section>$valor[idVenta]</section>";
                    print "<section>$valor[idUsuario]</section>";
                    print "<section>$valor[fecha]</section>";
                    print "<section>$valor[total]</section>";
                }
				print "</main>";
