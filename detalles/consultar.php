...
<body>
    ...
    <a href='consultar.php'>Actualizar tabla...</a>
		<h1>Consulta de la tabla de Detalles </h1>
		<main class="grid-6">
			<section>Clave</section>     
			<section>idProducto</section>
			<section>idUsuario</section>
            <section>Cantidad</section>
            <section>Fecha</section>
			<section>Total</section>
			<?php
                require "../classes/classDetalles.php";
                if (!empty($_SESSION['isAdmin'])) {
                    $detalles = new ApiDetallesAdmin;
                    $total = $detalles->getAll();
                } else {
                    $detalles = new ApiDetalles;
                    $total = $detalles->getAll($_SESSION['idUsuario']);
                }
                foreach ($total["items"] as $clave => $valor) {
                    print "<section>$valor[idDetalles]</section>";
                    print "<section>$valor[idProducto]</section>";
                    print "<section>$valor[idUsuario]</section>";
                    print "<section>$valor[cantVenta]</section>";
                    print "<section>$valor[fechaVenta]</section>";
                    print "<section>$valor[totalVenta]</section>";
                }
				print "</main>";
